...
export const MAKO_CHANNELS: MakoChannel[] = [
  {
    ...
    tokenUrl:
      "https://mako-streaming.akamaized.net/n12/hls/live/2103938/k12/index.m3u8?b-in-range=0-1100",
    group: "live",
  },
  {
    id: "k12-live",
...
const BY_ID = new Map(MAKO_CHANNELS.map(c => [c.id.toLowerCase(), c]));

export function findMakoChannel(id: string): MakoChannel | undefined {
  return BY_ID.get(id.trim().toLowerCase());
}

export function listMakoChannels(group?: MakoChannel["group"]): MakoChannel[] {
  if (!group) return [...MAKO_CHANNELS];
  return MAKO_CHANNELS.filter(c => c.group === group);
}

export function makoChannelPageUrl(id: string): string {
  return `mako:${id}`;
}

export function makoListingUrl(group?: string): string {
