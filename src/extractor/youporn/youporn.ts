...
const VALID_URL =
  /^https?:\/\/(?:www\.)?youporn\.com\/(?:watch|embed)\/(?<id>\d+)(?:\/(?<display_id>[^/?#&]+))?\/?(?:[#?]|$)/i;
...
export class YouPornIE extends InfoExtractor {
  ...
  private setAgeVerifiedCookie(): void {
  ...
  private parsePlayerVars(webpage: string, videoId: string): PlayerVars {
    ...
    const parsed = extractJsonObject(webpage, brace);
    if (!parsed || typeof parsed !== "object") {
      throw new Error(`youporn: could not parse player vars for ${videoId}`);
    }
    return parsed as PlayerVars;
  }

  private async remoteFormats(
    infoUrl: string,
    streamType: string,
    referer: string,
  ): Promise<RemoteFormat[]> {
    try {
      const data = await this.request.json<RemoteFormat[] | RemoteFormat>(infoUrl, {
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    if (!/<div[^>]*\bid=["']?watch-container/i.test(webpage)) {
      const msg =
        webpage.match(/id=["']mainContent["'][^>]*>([\s\S]*?)<\/div>/i)?.[1]?.replace(/<[^>]+>/g, " ").trim() ||
        "Video unavailable";
  ...
  async listVideos(url: string, options: ListVideosOptions = {}): Promise<VideoListResult> {
    ...
    let entries = parseYouPornWatchEntries(webpage, fetchUrl);
