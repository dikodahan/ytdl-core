...
const VALID_URL =
  /https?:\/\/(?:www\.)?dropbox\.com\/(?:(?:e\/)?scl\/f[io]|sh?)\/(?<id>\w+)/i;

function decodePrefetchParts(webpage: string): string[] {
  const parts: string[] = [];
  for (const m of webpage.matchAll(
    /registerStreamedPrefetch\s*\(\s*"[\w/+=]+"\s*,\s*"([\w/+=]+)"/g,
  )) {
    try {
...
export class DropboxIE extends InfoExtractor {
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    if (contentId) {
      ...
      if (typeof password !== "string" || !password) {
        throw new Error(
          "Password protected Dropbox video — pass extractorArgs.videopassword",
        );
      }
      ...
      const auth = await this.request.json<{ status?: string }>(
      ...
      if (auth.status !== "authed") throw new Error("Invalid Dropbox password");
    ...
    if (hasAnonymousDownload || !formats.length) {
      formats.push(
        progressiveFormat(dlUrl.toString(), {
          format_id: "original",
    ...
    if (!formats.length) throw new Error(`No playable formats for Dropbox ${id}`);

    const fn = urlBasename(url);
    const title = fn.replace(/\.[^.]+$/, "") || id;

    return baseInfo(DropboxIE.IE_NAME, url, {
      id,
      title,
      thumbnail,
      formats,
