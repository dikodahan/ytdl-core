...
const VALID_URL =
  /^https?:\/\/(?:www\.)?(?:bsky\.app|main\.bsky\.dev)\/profile\/(?<handle>[\w.:%-]+)\/post\/(?<id>\w+)/i;
...
interface BlueskyPost {
  ...
  record?: { text?: string; embed?: { media?: BlueskyEmbed; video?: unknown } };
  embed?: BlueskyEmbed & {
    media?: BlueskyEmbed;
    record?: { record?: BlueskyPost; embeds?: BlueskyEmbed[] };
  };
}

interface ThreadResponse {
  thread?: { post?: BlueskyPost };
}

function findPlaylist(embed: BlueskyEmbed | undefined | null): {
  playlist?: string;
  thumbnail?: string;
  alt?: string;
} {
...
export class BlueskyIE extends InfoExtractor {
  ...
  private async resolveDid(handle: string): Promise<string> {
    if (handle.startsWith("did:")) return handle;
    const res = await this.request.json<{ did?: string }>(
      "https://public.api.bsky.app/xrpc/com.atproto.identity.resolveHandle",
      { query: { handle } },
    );
    if (!res.did) throw new Error(`Could not resolve Bluesky handle: ${handle}`);
    return res.did;
  }
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    const data = await this.request.json<ThreadResponse>(
    ...
    if (!post) throw new Error(`Bluesky post not found: ${rkey}`);

    let found = findPlaylist(post.embed);
    if (!found.playlist) found = findPlaylist(post.embed?.media);
    if (!found.playlist) {
      const nested = post.embed?.record?.record;
      found = findPlaylist(nested?.embed);
      if (!found.playlist) found = findPlaylist(nested?.embed?.media);
      if (!found.playlist) {
        const embeds = (post.embed?.record as { embeds?: BlueskyEmbed[] } | undefined)?.embeds;
        if (embeds?.[0]) found = findPlaylist(embeds[0]);
      }
    }
    ...
    const formats: Format[] = [hlsFormat(found.playlist, "hls")];
