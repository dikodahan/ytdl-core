...
const VALID_URL = new RegExp(
  `^https?:\\/\\/(?:www\\.)?(?:bbc\\.(?:com|co\\.uk)|bbcnewsd73hkzno2ini43t4gblxvycyac5aw4gnv7t2rccijh7745uqd\\.onion|bbcweb3hytmzhn5d532owbu6oqadra5z3ar726vq5kgwwn6aucdccrad\\.onion)/(?:[^/]+/)+(?<id>[^/#?]+)`,
  "i",
);
...
interface BbcMedia {
  ...
  connection?: BbcMediaConnection[];
}

interface BbcMediaSelection {
  result?: string;
  media?: BbcMedia[];
}
...
export class BbcIE extends InfoExtractor {
  static IE_NAME = "bbc";
  ...
  private async downloadMediaSelector(vpid: string): Promise<Format[]> {
    ...
    for (const mediaSet of MEDIA_SETS) {
      ...
      let selection: BbcMediaSelection;
      try {
        const res = await this.request.request(url);
        selection = tryParseJson<BbcMediaSelection>(res.body) || {};
      ...
      for (const media of selection.media || []) {
        ...
        for (const conn of media.connection || []) {
          ...
          if (conn.transferFormat === "hls" || /\.m3u8(\?|$)/i.test(href)) {
            formats.push(hlsFormat(href, `hls-${conn.supplier || mediaSet}`));
          } else if (/^https?:/i.test(href) && !/\.f4m(\?|$)/i.test(href)) {
            formats.push(
              progressiveFormat(href, {
    ...
    if (!formats.length) {
      if (lastGeo) {
        throw new Error(
          `bbc media ${vpid} is geo-restricted (UK only); media selector: ${lastGeo}`,
        );
      }
  ...
  private findVpid(webpage: string): string | null {
    ...
    if (playable?.[2]) {
      const data = tryParseJson<{
        settings?: {
          playlistObject?: { items?: Array<{ vpid?: string; duration?: number }> };
        };
      }>(unescapeHtml(playable[2]));
      const vpid = data?.settings?.playlistObject?.items?.[0]?.vpid;
      if (vpid && pidRe.test(vpid)) return vpid;
    }
    ...
    for (const re of patterns) {
      ...
      if (m?.[1] && new RegExp(`^${PID}$`).test(m[1])) return m[1];
    }

    const pathPid = webpage.match(
      new RegExp(`/(?:iplayer/(?:episode|playlist)|programmes)/(${PID})`, "i"),
    );
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    const err = webpage.match(
      /<div\b[^>]+\bclass=["'](?:smp|playout)__message[^"']*["'][^>]*>\s*([^<]+)/i,
    )?.[1]?.trim();
    if (err) throw new Error(err);

    let vpid = this.findVpid(webpage);
    const urlPid = url.match(new RegExp(`\\/(?:episode|programmes)\\/(${PID})`, "i"))?.[1];
    if (!vpid && urlPid) vpid = urlPid;
    if (!vpid) throw new Error(`Could not find BBC video id (vpid) on ${displayId}`);

    const formats = await this.downloadMediaSelector(vpid);
    const title =
      og(webpage, "title")?.replace(/\s*-\s*BBC.*$/i, "").trim() ||
      webpage.match(/<h1[^>]*>([^<]+)/i)?.[1]?.trim() ||
      vpid;

    return baseInfo("bbc", url, {
      id: vpid,
