...
const EMBED_URL =
  /^https?:\/\/(?:www\.)?rumble\.com\/embed\/(?:[^-]+-)?(?<id>v[a-zA-Z0-9]+)/i;
const PAGE_URL =
  /^https?:\/\/(?:www\.)?rumble\.com\/v(?!ideos)(?<id>[^.?#]+)/i;
const VALID_URL =
  /^https?:\/\/(?:www\.)?rumble\.com\/(?:embed\/(?:[^-]+-)?(?<embed_id>v[a-zA-Z0-9]+)|v(?!ideos)(?<page_id>[^.?#]+))/i;

interface RumbleStreamMeta {
  h?: number | string;
  w?: number | string;
  bitrate?: number;
  size?: number;
...
interface RumbleStreamInfo {
  url?: string;
  meta?: RumbleStreamMeta;
}
...
interface RumbleVideo {
  ...
  i?: string;
  fps?: number;
  ua?: Record<string, Record<string, RumbleStreamInfo> | RumbleStreamInfo[]>;
...
export class RumbleIE extends InfoExtractor {
  ...
  private async resolveEmbedId(url: string): Promise<string> {
    const embed = url.match(EMBED_URL);
    if (embed?.groups?.id) return embed.groups.id;

    const page = url.match(PAGE_URL);
    if (!page) throw new Error(`Could not extract id from URL: ${url}`);

    const webpage = await this.request.text(url);
    const fromEmbed =
      webpage.match(/rumble\.com\/embed\/(?:[^-/"']+-)?(v[a-zA-Z0-9]+)/i)?.[1] ||
      webpage.match(/["']embedUrl["']\s*:\s*["'][^"']*\/embed\/(?:[^-/"']+-)?(v[a-zA-Z0-9]+)/i)?.[1] ||
      webpage.match(/Rumble\(\s*"play"\s*,\s*\{[^}]*["']?video["']?\s*:\s*["'](v?[0-9a-z]+)["']/i)?.[1];

    if (!fromEmbed) throw new Error(`Could not find Rumble embed id on page ${page.groups?.id}`);
    return fromEmbed.startsWith("v") ? fromEmbed : `v${fromEmbed}`;
  }

  async extract(url: string): Promise<InfoDict> {
    const videoId = await this.resolveEmbedId(url);
    const video = await this.request.json<RumbleVideo>("https://rumble.com/embedJS/u3/", {
    ...
    for (const [formatType, formatInfo] of Object.entries(video.ua || {})) {
      ...
      const items: RumbleStreamInfo[] = Array.isArray(formatInfo)
        ? formatInfo
        : Object.entries(formatInfo as Record<string, RumbleStreamInfo>).map(([height, info]) => {
            const meta = { ...(info.meta || {}) };
            if (meta.h == null) meta.h = Number(height) || height;
            return { ...info, meta };
          });
    ...
    if (!formats.length) throw new Error(`No playable formats for Rumble video ${videoId}`);

    return baseInfo("rumble", url, {
      id: videoId,
