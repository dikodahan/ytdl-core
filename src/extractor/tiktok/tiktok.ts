...
const VALID_URL =
  /^https?:\/\/(?:www\.)?tiktok\.com\/(?:embed|@(?<user>[\w.-]+)\/video)\/(?<id>\d+)/i;

function collectPlayAddrs(node: unknown, out: string[] = []): string[] {
  if (!node || typeof node !== "object") return out;
  if (Array.isArray(node)) {
    for (const item of node) collectPlayAddrs(item, out);
    return out;
  }
  ...
  if (typeof playAddr === "string" && playAddr.startsWith("http")) {
    ...
    const pa = playAddr as { url_list?: string[]; UrlList?: string[]; src?: string };
    for (const u of pa.url_list || pa.UrlList || []) {
      if (typeof u === "string") out.push(u);
    }
    if (typeof pa.src === "string") out.push(pa.src);
  }
  ...
  for (const v of Object.values(obj)) {
    if (v && typeof v === "object") collectPlayAddrs(v, out);
  }
  return out;
}

function findVideoMeta(node: unknown): {
  title?: string;
  author?: string;
  duration?: number;
  cover?: string;
  ...
  const walk = (n: unknown, depth = 0): void => {
    ...
    if (Array.isArray(n)) {
      for (const i of n) walk(i, depth + 1);
      return;
    }
    const o = n as Record<string, unknown>;
    if (typeof o.statusCode === "number") result.statusCode = o.statusCode;
    if (typeof o.desc === "string" && !result.title) result.title = o.desc;
    if (typeof o.nickname === "string" && !result.author) result.author = o.nickname;
    if (typeof o.uniqueId === "string" && !result.author) result.author = o.uniqueId;
    ...
    if (video) {
      ...
      if (typeof cover === "string") result.cover = cover;
    }
    const author = o.author as Record<string, unknown> | undefined;
    if (author && typeof author.nickname === "string") result.author = author.nickname;
...
export class TiktokIE extends InfoExtractor {
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    if (/Please wait|Verify to continue|captcha/i.test(webpage) && webpage.length < 5000) {
      throw new Error(
        "TikTok blocked this request (captcha / bot check). Try cookies via agent or impersonate.",
      );
    }
    ...
    let state: unknown =
      ...
      tryParseJson(
        extractBetween(webpage, 'id="SIGI_STATE" type="application/json">', "</script>") ||
          extractBetween(webpage, 'id="sigi-persisted-data">', "</script>") ||
          "",
      );

    if (!state) {
      const uni = webpage.match(
        /<script[^>]+id="__UNIVERSAL_DATA_FOR_REHYDRATION__"[^>]*>(\{[\s\S]*?\})<\/script>/,
      );
      if (uni?.[1]) state = tryParseJson(uni[1]);
    }
    if (!state) {
      const sigi = webpage.match(
        /<script[^>]+id="(?:SIGI_STATE|sigi-persisted-data)"[^>]*>(\{[\s\S]*?\})<\/script>/,
      );
      if (sigi?.[1]) state = tryParseJson(sigi[1]);
    }

    if (!state) {
      throw new Error(
        "Unable to parse TikTok page state (may be blocked). Try cookies via agent or forceImpersonate.",
      );
    }
    ...
    const formats: Format[] = addrs.map((u, i) =>
      progressiveFormat(u, {
