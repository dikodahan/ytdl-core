...
const VALID_URL =
  /^https?:\/\/(?:www\.)?reuters\.com\/(?:.*?[?&]videoId=(?<id>\d+)|video\/(?:[^/?#]+\/)*(?<slug>[^/?#]+)|(?:[^/?#]+\/)*video\/(?<slug2>[^/?#]+))/i;
...
export class ReutersIE extends InfoExtractor {
  ...
  private async extractLegacy(videoId: string, pageUrl: string): Promise<InfoDict | null> {
    let iframe: string;
    try {
      iframe = await this.request.text(
        `https://www.reuters.com/assets/iframe/yovideo?videoId=${videoId}`,
      );
    } catch {
      return null;
    }
    ...
    let mas: MasFormat[] = [];
    try {
      const masRaw = await this.request.text(
        `https://mas-e.cds1.yospace.com/mas/${ids[1]}/${ids[2]}?trans=json`,
      );
      mas = tryParseJson<MasFormat[]>(masRaw.replace(/'/g, '"')) || [];
    } catch {
      return null;
    }
    ...
    for (const f of mas) {
      if (!f.url) continue;
      if (f.method === "hls" || /\.m3u8(\?|$)/i.test(f.url)) formats.push(hlsFormat(f.url));
      else {
        formats.push(
    ...
    return baseInfo("reuters", pageUrl, {
      id: videoId,
      title,
      thumbnail: thumb,
  ...
  private extractFromWebpage(webpage: string, url: string, id: string): InfoDict {
    const formats: Format[] = [];
    const seen = new Set<string>();

    const jsonLdBlocks = [...webpage.matchAll(
      /<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi,
    )];
    for (const block of jsonLdBlocks) {
      const data = tryParseJson<Record<string, unknown> | Array<Record<string, unknown>>>(
        block[1] || "",
      );
      ...
      for (const node of nodes) {
        ...
        if (contentUrl && !seen.has(contentUrl)) {
          ...
          if (/\.m3u8(\?|$)/i.test(contentUrl)) formats.push(hlsFormat(contentUrl));
            ...
            formats.push(progressiveFormat(contentUrl));
          }
        }
        ...
        if (Array.isArray(encodings)) {
          for (const enc of encodings) {
            const u = enc && typeof enc === "object" && "contentUrl" in enc
              ? String((enc as { contentUrl?: string }).contentUrl || "")
              : "";
            if (!u || seen.has(u)) continue;
            seen.add(u);
            if (/\.m3u8(\?|$)/i.test(u)) formats.push(hlsFormat(u));
            else if (/^https?:/i.test(u)) formats.push(progressiveFormat(u));
          }
        }
      }
    }
    ...
    for (const m of webpage.matchAll(/https?:\/\/[^"'\\\s>]+\.m3u8[^"'\\\s>]*/gi)) {
      ...
      seen.add(u);
      formats.push(hlsFormat(u));
    }
    for (const m of webpage.matchAll(/https?:\/\/[^"'\\\s>]+\.mp4[^"'\\\s>]*/gi)) {
      const u = unescapeHtml(m[0]);
      if (seen.has(u) || /thumbnail|sprite|poster/i.test(u)) continue;
      seen.add(u);
      formats.push(progressiveFormat(u));
    }

    if (!formats.length) {
      throw new Error(
        `Reuters video ${id} has no playable HLS/MP4 (legacy yovideo API unavailable)`,
      );
    }

    return baseInfo("reuters", url, {
      id,
  ...
  async extract(url: string): Promise<InfoDict> {
    ...
    if (!videoId && !slug) throw new Error(`Could not extract id from URL: ${url}`);
    ...
    const idFromPage =
      webpage.match(/["']videoId["']\s*:\s*["']?(\d+)/i)?.[1] ||
      webpage.match(/videoId=(\d+)/i)?.[1] ||
      videoId ||
      slug ||
      matchId(url, VALID_URL);
