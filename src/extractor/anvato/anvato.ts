...
const MCP_TO_ACCESS_KEY: Record<string, string> = {
...
const ANVACK_TABLE: Record<string, string> = {
  ...
  anvato_mcp_univision_web_prod_37fe34850c99a3b5cdb71dab10a417dd5cdecafa:
    "bLDYF8JqfG42b7bwKEgQiU9E2LTIAtnKzSgYpFUH",
  ...
  anvato_mcp_telemundo_web_prod_c5278d51ad46fda4b6ca3d0ea44a7846a054f582:
    "qyT6PXXLjVNCrHaRVj0ugAhalNRS7Ee9BP7LUokD",
...
interface AnvatoVideoData {
  published_urls?: AnvatoPublishedUrl[];
  def_title?: string;
...
function anvatoAdstAuth(inputData: string): string {
...
export class AnvatoIE extends InfoExtractor {
  static IE_NAME = "anvato";
  static IE_DESC = "Anvato MCP video (access-key URLs)";
  static readonly _VALID_URL = /anvato:(?<access>[^:]+):(?<id>\d+)/i;

  async extract(url: string): Promise<InfoDict> {
    const m = url.match(AnvatoIE._VALID_URL);
    if (!m?.groups) throw new Error(`Could not parse Anvato URL: ${url}`);
    ...
    try {
      const st = await this.request.json<{ server_time?: number }>(
        `${API_BASE}/server_time`,
        { query: { anvack: accessKey } },
      );
    ...
    if (res.statusCode >= 400) {
      throw new Error(
        `Anvato MCP video ${videoId} failed (HTTP ${res.statusCode}). ` +
    ...
    let videoData = tryParseJson<AnvatoVideoData>(stripJsonp(res.body));
    if (!videoData) {
    ...
    for (const pub of videoData.published_urls || []) {
      ...
      const isM3u8 =
        mediaFormat.includes("m3u8") || /\.m3u8(\?|$)/i.test(videoUrl);

      if (isM3u8) {
        // Some hosts return JSON with master_m3u8
        try {
          const manifest = await this.request.json<AnvatoVideoData>(videoUrl);
          if (manifest.master_m3u8) videoUrl = manifest.master_m3u8;
        } catch {
          /* use original */
        }
        formats.push(hlsFormat(videoUrl!, mediaFormat.includes("variant") ? "hls" : `hls-${pub.kbps || ""}`));
    ...
    if (!formats.length) {
      throw new Error(
        `Anvato video ${videoId} has no published URLs. ` +
          `Verify the access key is valid (see Anvato access keys / MCP table).`,
      );
    }

    return baseInfo(AnvatoIE.IE_NAME, url, {
      id: videoId,
