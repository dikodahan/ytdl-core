...
export const URL_USAGE: Record<string, UrlUsageGuide> = {
  youtube: {
    usage:
      "Paste a watch / Shorts / youtu.be / Music / live URL, or an 11-character video id.",
    examples: [
  ...
  brightcove: {
    usage:
      "Paste a Brightcove player page URL (`players.brightcove.net/.../index.html?videoId=…`).",
    examples: [
  ...
  jwplatform: {
    usage:
      "Paste a JW Platform / JW Player media or player URL (8-character media id), or `jwplatform:ID`.",
    examples: [
  ...
  kaltura: {
    usage:
      "Paste a Kaltura widget/entry URL, or the pseudo-URL `kaltura:PARTNER_ID:ENTRY_ID`.",
    examples: [
  ...
  "kaltura-ott": {
    usage:
      "Paste a Kaltura OTT pseudo-URL for live channels, EPG listings, or catch-up programs. Built-in presets: `reshet`, `cellcom`.",
    examples: [
    ...
    notes:
      "List channels with `:channels` or `:lineup:{id}`. List EPG with `:epg:{channelId}` (asset id for Reshet, epg channel id for Cellcom). Add `?days=4` for EPG range.",
  },
  anvato: {
  ...
  theplatform: {
    usage:
      "Paste a thePlatform link or player media URL (`link.theplatform.com/s/…` or `player.theplatform.com/…`).",
    examples: [
  ...
  cloudflarestream: {
    usage:
      "Paste a Cloudflare Stream watch / iframe / customer subdomain URL (32-hex id or signed JWT).",
    examples: [
  ...
  bunnycdn: {
    usage:
      "Paste a Bunny Stream embed or play URL from `iframe.mediadelivery.net` / `player.mediadelivery.net`.",
    examples: [
  ...
  peertube: {
    usage:
      "Paste a PeerTube watch/embed URL from any instance (`/w/{uuid}` or `/videos/watch/{uuid}`), or `peertube:host:uuid`.",
    examples: [
  ...
  audiomack: {
    usage:
      "Paste an Audiomack song URL (`/song/{artist}/{slug}` or `/{artist}/song/{slug}`).",
    examples: [
  ...
  xnxx: {
    usage:
      "Paste an XNXX video URL (`xnxx.com/video-{id}/…`) from www or video subdomains.",
    examples: [
  ...
  aljazeera: {
    usage:
      "Paste a channel pseudo-URL (`aljazeera:english`), network channel page, live page, or dated article/video URL.",
    examples: [
      "aljazeera:english",
...
export const LIST_URL_USAGE: Record<string, UrlUsageGuide> = {
  ...
  xnxx: {
    usage:
      "Paste an XNXX browse URL: homepage, search/tag page (`/search/{term}/`), best, hits, or porn-maker listing.",
    examples: [
  ...
  "kaltura-ott": {
    usage:
      "Paste a Kaltura OTT listing pseudo-URL: `:channels`, `:lineup:{id}`, or `:epg:{channelId}?days=4`.",
    examples: [
    ...
    notes:
      "Channel list returns live asset ids. EPG list returns program ids suitable for `:program:{id}` extraction. Reshet EPG uses linear asset ids; Cellcom accepts epg channel id (externalIds).",
  },
};
