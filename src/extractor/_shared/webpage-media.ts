...
import { baseInfo, hlsFormat, progressiveFormat, tryParseJson } from "./helpers";
...
function pushUrl(formats: Format[], raw: string | undefined | null, base: string, formatId: string): void {
  if (!raw) return;
  const url = absUrl(unescapeHtml(raw.trim()), base);
  if (!url || !/^https?:\/\//i.test(url)) return;
  if (formats.some(f => f.url === url || f.manifest_url === url)) return;
...
export async function extractWebpageMedia(
  ...
  const ogVideo =
    html.match(/property=["']og:video(?::(?:secure_url|url))?["'][^>]*content=["']([^"']+)["']/i)?.[1] ||
    html.match(/content=["']([^"']+)["'][^>]*property=["']og:video(?::(?:secure_url|url))?["']/i)?.[1];
  pushUrl(formats, ogVideo, pageUrl, "og-video");

  const twitterPlayer =
    html.match(/name=["']twitter:player:stream["'][^>]*content=["']([^"']+)["']/i)?.[1] ||
  ...
  for (const m of html.matchAll(
    ...
    const data = tryParseJson<unknown>(m[1]!.trim());
  ...
  const id =
    html.match(/property=["']og:url["'][^>]*content=["']([^"']+)["']/i)?.[1]?.split("/").filter(Boolean).pop() ||
    new URL(pageUrl).pathname.split("/").filter(Boolean).pop() ||
    extractorName;

  if (!formats.length) {
    throw new Error(
      `${extractorName}: no playable media found on page (site may need a dedicated extractor or cookies)`,
    );
  }

  return baseInfo(extractorName, pageUrl, {
...
function walkJsonLd(node: unknown, pageUrl: string, formats: Format[]): void {
  if (!node || typeof node !== "object") return;
  if (Array.isArray(node)) {
    for (const n of node) walkJsonLd(n, pageUrl, formats);
    return;
  }
  ...
  if (/VideoObject|AudioObject|MediaObject/i.test(type)) {
    const content =
      (typeof obj.contentUrl === "string" && obj.contentUrl) ||
      (typeof obj.embedUrl === "string" && obj.embedUrl) ||
      (typeof obj.url === "string" && obj.url) ||
      null;
    pushUrl(formats, content, pageUrl, "jsonld");
  }
  if (obj["@graph"]) walkJsonLd(obj["@graph"], pageUrl, formats);
  for (const v of Object.values(obj)) {
    if (v && typeof v === "object") walkJsonLd(v, pageUrl, formats);
  }
}
