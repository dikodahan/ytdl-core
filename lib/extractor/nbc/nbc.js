...
exports.NbcIE = void 0;
const info_extractor_1 = require("../../core/info-extractor");
const helpers_1 = require("../_shared/helpers");
const VALID_URL = /^https?(?<permalink>:\/\/(?:www\.)?nbc\.com\/(?:classic-tv\/)?[^/?#]+\/video\/[^/?#]+\/(?<id>\w+))/i;
const M3U8_DRM_RE = /https?:\/\/[^/?#]+\/prod\/[\w-]+\/(?<folders>[^?#]+\/)cmaf\/mpeg_(?:cbcs|cenc)\w*\/master_cmaf\w*\.m3u8/i;
function extractSmilSrc(smil) {
    const m = smil.match(/<(?:video|ref)\b[^>]*\bsrc=["']([^"']+)["']/i);
    return m?.[1] || null;
}
function extractSmilError(smil) {
...
class NbcIE extends info_extractor_1.InfoExtractor {
    static IE_NAME = "nbc";
    ...
    async downloadNbcuM3u8(tpPath, query) {
        ...
        if (!src)
            throw new Error("NBC SMIL response contained no video src");
    ...
    async extract(url) {
        ...
        const gql = await this.request.json("https://friendship.nbc.co/v2/graphql", {
            query: {
  ...
  bonanzaPage(app: $app, name: $name, oneApp: $oneApp, platform: $platform, type: $type, userId: $userId) {
    metadata { ... on VideoPageData { description episodeNumber locked mpxAccountId mpxGuid rating resourceId seasonNumber secondaryTitle seriesShortTitle } }
        ...
        if (videoData.locked) {
            throw new Error("NBC video requires cable/MVPD authentication (DRM/auth); unsupported for VLC-only extract");
        }
        ...
        const formats = [(0, helpers_1.hlsFormat)(m3u8Url)];
        ...
        let duration = null;
        try {
            const preview = await this.request.json(`https://link.theplatform.com/s/${tpPath}`, { query: { format: "preview" } });
        ...
        return (0, helpers_1.baseInfo)("nbc", url, {
            id: videoId,
            title,
            description,
            thumbnail,
