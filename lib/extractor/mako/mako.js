...
class MakoIE extends info_extractor_1.InfoExtractor {
    ...
    static getInfo() {
        return {
            ...
            options: [],
            notes: "Use `mako:channels` to list IDs (discovered from mako.co.il, MediaBox fallback), or `mako:k12` / a `mako-streaming.akamaized.net` m3u8 URL to extract. Streams require a short-lived Akamai `hdnea` ticket.",
            listSupported: true,
        };
    }
    static suitable(url) {
    ...
    async resolveTarget(url) {
        ...
        const { channels } = await (0, discover_1.getMakoCatalog)(this.request);
        const channel = (0, discover_1.findInMakoCatalog)(channels, id) || (0, channels_1.findMakoChannel)(id) || (0, channels_1.listMakoChannels)().find(c => c.id === id);
        if (!channel)
