...
exports.collectLiveTvEntries = collectLiveTvEntries;
exports.stableIdForSiteChannel = stableIdForSiteChannel;
exports.groupForChannel = groupForChannel;
exports.discoverMakoChannelsFromSite = discoverMakoChannelsFromSite;
exports.selectMakoCatalog = selectMakoCatalog;
exports.mergeMakoCatalog = mergeMakoCatalog;
exports.isMakoStreamPlayable = isMakoStreamPlayable;
exports.filterDeadFallbackChannels = filterDeadFallbackChannels;
exports.getMakoCatalog = getMakoCatalog;
exports.findInMakoCatalog = findInMakoCatalog;
exports.clearMakoDiscoveryCache = clearMakoDiscoveryCache;
            ...
            if (depth <= 2 && !seen.has(path)) {
                ...
                out.push({
                    ...
                    title: (typeof rec.title === "string" && rec.title) ||
                        ...
                        undefined,
                    itemVcmId: (typeof rec.itemVcmId === "string" && rec.itemVcmId) ||
...
function streamSegment(streamUrl) {
    try {
...
function stableIdForSiteChannel(opts) {
    const slug = opts.pageUrl.split("/").filter(Boolean).pop() || "";
    if (PATH_SLUG_TO_ID[slug])
        return PATH_SLUG_TO_ID[slug];
    if (opts.streamUrl) {
        const seg = streamSegment(opts.streamUrl);
        if (seg && STREAM_SEGMENT_TO_ID[seg])
            return STREAM_SEGMENT_TO_ID[seg];
        if (seg) {
            return seg
                .toLowerCase()
                .replace(/_/g, "-")
...
function groupForChannel(id, streamUrl) {
    if (id.startsWith("free-") || /\/free_/i.test(streamUrl))
        return "free";
    if (id === "k12" ||
        id === "k12cc" ||
        id === "ch24" ||
...
function contentUrlFromPlaylistProps(playlist) {
    if (!playlist)
        return undefined;
    const seo = playlist.seo;
    const schema = seo?.schema;
    const video = schema?.video;
    const contentUrl = video?.contentUrl;
    return typeof contentUrl === "string" ? contentUrl : undefined;
}
function thumbnailFromPlaylistProps(playlist) {
    if (!playlist)
        return undefined;
    const hero = playlist.hero;
    const pics = hero?.pics;
    if (pics?.[0]?.picUrl)
        return pics[0].picUrl;
    const seo = playlist.seo;
    if (typeof seo?.image === "string")
        return seo.image;
    return undefined;
}
function titleFromPlaylistProps(playlist, fallback) {
    if (playlist) {
        const hero = playlist.hero;
...
async function fetchNextData(request, url) {
    const html = await request.text(url, { headers: token_1.MAKO_REQUEST_HEADERS });
    return parseNextData(html);
}
async function resolveStreamViaPlaylistApi(request, vod) {
    ...
    if (!itemVcmId || !channelId)
        return {};
    const galleryChannelId = (typeof vod.galleryChannelId === "string" && vod.galleryChannelId) || itemVcmId;
    ...
    try {
        const data = await request.json(`${MAKO_ORIGIN}/AjaxPage?${qs.toString()}`, { headers: token_1.MAKO_REQUEST_HEADERS });
        return {
...
async function resolveSiteChannel(request, entry) {
    ...
    const playlist = pageProps?.playlist;
    ...
    if (!streamUrl) {
        ...
        if (episodePath) {
            try {
                ...
                const epProps = epData?.props
                    ?.pageProps;
                const epPlaylist = epProps?.playlist;
    ...
    if (!streamUrl || !/mako-streaming\.akamaized\.net/i.test(streamUrl))
        return null;
...
async function discoverMakoChannelsFromSite(request) {
 ...
 */
function selectMakoCatalog(siteChannels, fallback = channels_1.MAKO_CHANNELS) {
...
function mergeMakoCatalog(siteChannels, fallback = channels_1.MAKO_CHANNELS) {
    return selectMakoCatalog(siteChannels, fallback).channels;
}
/** True when a tokenized Mako HLS playlist is reachable. */
async function isMakoStreamPlayable(request, streamUrl, tokenUrl) {
    try {
        const ticket = await (0, token_1.fetchMakoTicket)(request, tokenUrl || streamUrl);
        const playUrl = (0, token_1.buildAuthorizedMakoUrl)(streamUrl, ticket);
        const res = await request.request(playUrl, { headers: token_1.MAKO_REQUEST_HEADERS });
...
async function getMakoCatalog(request, options = {}) {
    ...
    const selected = selectMakoCatalog(site, channels_1.MAKO_CHANNELS);
    const channelsAlive = selected.source === "fallback"
