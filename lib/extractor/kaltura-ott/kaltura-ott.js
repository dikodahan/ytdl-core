...
class KalturaOttIE extends info_extractor_1.InfoExtractor {
    ...
    client(preset) {
        return new client_1.KalturaOttClient(this.request, preset, this.credentials());
    }
    credentials() {
        const args = this.params.extractorArgs?.kalturaOtt;
        const username = typeof args?.username === "string" ? args.username.trim() : "";
        const password = typeof args?.password === "string" ? args.password : "";
        if (!username && !password)
            return undefined;
        if (!username || !password) {
            throw new client_1.KalturaOttAuthenticationError("Kaltura OTT username and password must both be supplied.");
        }
    ...
    async extractLive(url, parsed, preset, client, assetId) {
        ...
        for (const source of sources) {
            ...
            if (type.includes("DASH") || source.url.includes(".mpd")) {
                formats.push((0, helpers_1.dashFormat)(source.url, `dash-${formats.length + 1}`));
            }
            else {
                formats.push((0, helpers_1.hlsFormat)(source.url, `hls-${formats.length + 1}`));
            }
        }
        if (!formats.length) {
