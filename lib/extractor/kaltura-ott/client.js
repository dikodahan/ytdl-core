...
exports.KalturaOttClient = exports.KalturaOttSubscriptionRequiredError = exports.KalturaOttAuthenticationError = void 0;
exports.pickChannelLogo = pickChannelLogo;
exports.pickStreamFormats = pickStreamFormats;
class KalturaOttAuthenticationError extends Error {
    code = "KALTURA_OTT_AUTHENTICATION_FAILED";
    statusCode = 401;
...
exports.KalturaOttAuthenticationError = KalturaOttAuthenticationError;
class KalturaOttSubscriptionRequiredError extends Error {
    code = "KALTURA_OTT_SUBSCRIPTION_REQUIRED";
    statusCode = 403;
...
exports.KalturaOttSubscriptionRequiredError = KalturaOttSubscriptionRequiredError;
...
class KalturaOttClient {
    ...
    credentials;
    session = null;
    constructor(request, preset, credentials) {
    ...
    async ensureSession() {
        ...
        if (result?.error) {
            ...
            throw new Error(`Kaltura OTT anonymous login failed${result.error.code ? ` (${result.error.code})` : ""}`);
        }
    ...
    async serveByDevice() {
        ...
        if (gateway)
            this.preset.apiHost = gateway.replace(/\/+$/, "");
    }
    apiBase() {
    ...
    async getLivePlayback(assetId) {
        const result = await this.postApi("/api_v3/service/asset/action/getPlaybackContext", {
            apiVersion: this.preset.apiVersion,
            clientTag: this.preset.clientTag,
            ignoreNull: true,
            format: 1,
        ...
        const blocked = result.actions?.some(action => action.type === "BLOCK" ||
