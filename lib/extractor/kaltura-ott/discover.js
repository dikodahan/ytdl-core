...
exports.PARTNER_ID_SCAN_MAX = exports.PARTNER_ID_SCAN_MIN = void 0;
...
exports.isDiscoveryMatch = isDiscoveryMatch;
exports.scanKalturaOttPartnerIds = scanKalturaOttPartnerIds;
exports.scrapePartnerCandidates = scrapePartnerCandidates;
const presets_1 = require("./presets");
exports.PARTNER_ID_SCAN_MIN = 1000;
exports.PARTNER_ID_SCAN_MAX = 9999;
...
async function discoverKalturaOttPartner(request, applicationNameOrUrl, options = {}) {
    ...
    if (!knownHit && !deepScan) {
        notes.push(`No built-in mapping for ${applicationName}. Create a discovery job to scan partner IDs ${exports.PARTNER_ID_SCAN_MIN}–${exports.PARTNER_ID_SCAN_MAX}.`);
    }
    ...
    if (!hasMatch() && deepScan) {
        notes.push(`Inline deep scan: probing up to ${deepScanLimit} partner IDs from ${exports.PARTNER_ID_SCAN_MIN} (prefer a discovery job for the full range).`);
        const tried = new Set(candidates.map(c => c.partnerId));
        for (let id = exports.PARTNER_ID_SCAN_MIN; id <= exports.PARTNER_ID_SCAN_MAX && probesAttempted < deepScanLimit; id++) {
            if (tried.has(id))
                continue;
            tried.add(id);
            probesAttempted++;
        ...
        if (!hasMatch()) {
            notes.push(`Inline deep scan did not find a partner ID in the first ${deepScanLimit} probes. Use a discovery job to continue through ${exports.PARTNER_ID_SCAN_MAX}.`);
        }
    }
 ...
 */
async function scanKalturaOttPartnerIds(request, applicationNameOrUrl, options = {}) {
    const started = Date.now();
    const notes = [];
    const applicationName = normalizeAndroidApplicationName(applicationNameOrUrl);
    const from = Math.max(exports.PARTNER_ID_SCAN_MIN, Math.floor(options.from ?? exports.PARTNER_ID_SCAN_MIN));
    const to = Math.min(exports.PARTNER_ID_SCAN_MAX, Math.floor(options.to ?? exports.PARTNER_ID_SCAN_MAX));
    if (to < from) {
    ...
    for (let id = from; id <= to; id++) {
        ...
        tried.add(id);
        probed++;
        hit = await probePartner(request, {
            partnerId: id,
            source: "range scan anonymousLogin",
            weight: 0,
            applicationName,
        }, applicationName, { loginOnly: true });
...
async function probePartner(request, candidate, applicationName, opts = {}) {
    ...
    if (opts.loginOnly) {
        const deviceOk = await probeServeByDeviceQuick(request, loginHit.apiHost, candidate.partnerId, applicationName, loginHit.apiVersion);
        if (!deviceOk) {
            return null;
        }
...
async function probeServeByDeviceQuick(request, apiHost, partnerId, applicationName, apiVersion) {
