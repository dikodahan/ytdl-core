...
class YouPornIE extends info_extractor_1.InfoExtractor {
    ...
    async listCategories(url = "https://www.youporn.com/", options = {}) {
        this.setAgeVerifiedCookie();
        const normalized = this.listingUrl(url);
        const targets = normalized === "https://www.youporn.com/" ||
        ...
        for (const indexUrl of targets) {
            try {
                ...
                let entries = (0, page_links_1.parseYouPornCategories)(webpage, indexUrl);
                if (!entries.length) {
