...
exports.YandexDiskIE = void 0;
const info_extractor_1 = require("../../core/info-extractor");
const helpers_1 = require("../_shared/helpers");
const VALID_URL = /https?:\/\/(?<domain>yadi\.sk|disk\.(?:360\.)?yandex\.(?:az|by|com(?:\.(?:am|ge|tr))?|co\.il|ee|fr|k[gz]|l[tv]|md|t[jm]|u[az]|ru))\/(?:[di]\/?|public.*?\bhash=)(?<id>[^/?#&]+)/i;
...
class YandexDiskIE extends info_extractor_1.InfoExtractor {
    ...
    async extract(url) {
        ...
        const storeRaw = webpage.match(/<script[^>]+id=["']store-prefetch["'][^>]*>\s*(\{[\s\S]+?\})\s*<\/script>/i)?.[1];
        const store = storeRaw ? (0, helpers_1.tryParseJson)(storeRaw) : null;
        ...
        if (!resource)
            throw new Error(`Yandex Disk resource missing for ${videoId}`);
        ...
        let sourceUrl;
        try {
            const dl = await this.request.json("https://cloud-api.yandex.net/v1/disk/public/resources/download", { query: { public_key: url } });
            sourceUrl = dl.href;
        }
        catch {
        ...
        if (sk && yandexuid && (!sourceUrl || !videoStreams.videos?.length)) {
            this.request.agent.jar.setCookieSync(`yandexuid=${yandexuid}; Path=/; Domain=${domain.startsWith("disk.") ? domain : `.${domain}`}`, `https://${domain}/`);
            const callApi = async (action) => {
                try {
                    const resp = await this.request.json(new URL(`/public/api/${action}`, url).toString(), {
                        method: "POST",
        ...
        const formats = [];
        if (sourceUrl) {
            formats.push((0, helpers_1.progressiveFormat)(sourceUrl, {
                format_id: "source",
        ...
        for (const video of videoStreams.videos || []) {
            ...
            if (video.dimension === "adaptive") {
                formats.push((0, helpers_1.hlsFormat)(video.url, "hls"));
            }
            else {
                const height = video.size?.height ?? null;
                formats.push((0, helpers_1.progressiveFormat)(video.url, {
...
exports.YandexDiskIE = YandexDiskIE;
//# sourceMappingURL=yandexdisk.js.map