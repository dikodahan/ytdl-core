...
exports.DEFAULT_ONTIVI_PLAYER_CONFIG = exports.DEFAULT_PLAYER_Y = void 0;
exports.decodePlayerjsString = decodePlayerjsString;
exports.decodePlayerjsFile2 = decodePlayerjsFile2;
exports.applyPlayerjsPlaceholders = applyPlayerjsPlaceholders;
exports.resolveOntiviPlayerFile = resolveOntiviPlayerFile;
exports.unpackPacker = unpackPacker;
exports.parseOntiviPlayerConfig = parseOntiviPlayerConfig;
...
function decodePlayerjsString(encoded, y = exports.DEFAULT_PLAYER_Y) {
    if (encoded.startsWith("#1"))
        return saltDecode(pepper(encoded.slice(2), -1, y));
    if (encoded.startsWith("#0"))
        return saltDecode(encoded.slice(2));
    return encoded;
}
...
function decodePlayerjsFile2(encoded, config) {
...
function applyPlayerjsPlaceholders(template, vars, config) {
    let out = template;
    for (let i = 1; i <= 5; i++) {
        const fpv = config[`fpv${i}`];
        if (!out.includes(`{v${i}}`) || typeof fpv !== "string" || !fpv)
            continue;
...
function resolveOntiviPlayerFile(file, vars, config = exports.DEFAULT_ONTIVI_PLAYER_CONFIG) {
    let cur = file.trim();
    for (let round = 0; round < 4; round++) {
        const enc2 = config.enc2 || "F";
        if (!cur.startsWith(`#${enc2}`) && !cur.startsWith("#2") && !cur.startsWith("#F"))
            break;
        const next = decodePlayerjsFile2(cur, config);
        if (!next || next === cur)
            break;
        cur = next;
        if (/https?:\/\//i.test(cur) && !cur.startsWith("#"))
            break;
    }
    if (cur.startsWith("#")) {
...
function unpackPacker(source) {
    ...
    if (!m)
        throw new Error("Not a packed Playerjs script");
    let payload = m[1];
    const radix = Number(m[2]);
    const count = Number(m[3]);
    const keywords = m[4].split("|");
    const encode = (c) => (c < radix ? "" : encode(Math.floor(c / radix))) +
...
function extractEscapedString(source, marker) {
...
function parseOntiviPlayerConfig(playerJs) {
    const unpacked = unpackPacker(playerJs);
    const y = extractEscapedString(unpacked, "y:\\'") ||
        unpacked.match(/y:'(xx\?\?\?x=xx\?xx\?=)'/)?.[1] ||
        exports.DEFAULT_PLAYER_Y;
    const uEnc = extractEscapedString(unpacked, "u:\\'") ||
        unpacked.match(/u:'(#1[^']+)'/)?.[1];
    if (!uEnc)
        return { ...exports.DEFAULT_ONTIVI_PLAYER_CONFIG };
    const raw = decodePlayerjsString(uEnc, y);
    const cfg = JSON.parse(raw);
    return {
        enc2: typeof cfg.enc2 === "string" ? cfg.enc2 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.enc2,
        file3_separator: typeof cfg.file3_separator === "string"
            ? cfg.file3_separator
            : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.file3_separator,
        bk0: typeof cfg.bk0 === "string" ? cfg.bk0 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.bk0,
        bk1: typeof cfg.bk1 === "string" ? cfg.bk1 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.bk1,
        bk2: typeof cfg.bk2 === "string" ? cfg.bk2 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.bk2,
        bk3: typeof cfg.bk3 === "string" ? cfg.bk3 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.bk3,
        bk4: typeof cfg.bk4 === "string" ? cfg.bk4 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.bk4,
        fpv1: typeof cfg.fpv1 === "string" ? cfg.fpv1 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.fpv1,
        fpv2: typeof cfg.fpv2 === "string" ? cfg.fpv2 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.fpv2,
        fpv3: typeof cfg.fpv3 === "string" ? cfg.fpv3 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.fpv3,
        fpv4: typeof cfg.fpv4 === "string" ? cfg.fpv4 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.fpv4,
        fpv5: typeof cfg.fpv5 === "string" ? cfg.fpv5 : exports.DEFAULT_ONTIVI_PLAYER_CONFIG.fpv5,
        fplace: typeof cfg.fplace === "number" ? cfg.fplace : 1,
    };
}
//# sourceMappingURL=playerjs.js.map