...
exports.PbsIE = void 0;
const info_extractor_1 = require("../../core/info-extractor");
const helpers_1 = require("../_shared/helpers");
const VALID_URL = /^https?:\/\/(?:(?:video|player)\.pbs\.org\/(?:widget\/)?partnerplayer\/(?<player_id>[^/?#]+)|(?:(?:www|video|player)\.)?pbs\.org\/(?:(?:vir|port)alplayer|video)\/(?<id>[0-9]+)(?:[?/#]|$)|(?:www\.)?(?:pbs|thirteen|wgbh|wnet)\.org\/(?:[^/?#]+\/){1,5}(?<presumptive_id>[^/?#]+?)(?:\.html)?\/?(?:$|[?#]))/i;
...
function extractVideoData(html) {
    const m = html.match(/PBS\.videoData\s*=\s*/) ||
...
class PbsIE extends info_extractor_1.InfoExtractor {
    static IE_NAME = "pbs";
    ...
    async resolveVideoId(url) {
        ...
        if (m.groups.player_id) {
            ...
            const videoId = player.match(/<div\s+id=["']video_(\d+)/i)?.[1] ||
                (data?.id != null ? String(data.id) : null) ||
                (data?.contentID != null ? String(data.contentID) : null);
            if (!videoId)
                throw new Error(`Could not find PBS video id in player ${m.groups.player_id}`);
        ...
        for (const re of mediaPatterns) {
            ...
            if (hit?.[1])
                return { videoId: hit[1], displayId, description };
        }
    ...
    async extract(url) {
        ...
        const seen = new Set();
        let info = {};
        for (const page of ["widget/partnerplayer", "portalplayer"]) {
            let player;
            try {
        ...
        for (const redirect of redirects) {
            ...
            if (!formatUrl)
                continue;
            if (/\.m3u8(\?|$)/i.test(formatUrl))
                formats.push((0, helpers_1.hlsFormat)(formatUrl));
            else
                formats.push((0, helpers_1.progressiveFormat)(formatUrl, { format_id: redirect.eeid || "http" }));
        }
        if (!formats.length) {
            if (geoError)
                throw new Error(`PBS video is geo-restricted to the US: ${geoError}`);
        ...
        const title = info.title
            ? program
                ...
                : info.title
            : videoId;
        return (0, helpers_1.baseInfo)("pbs", url, {
            id: videoId,
