...
exports.YoutubeIE = void 0;
const info_extractor_1 = require("../../core/info-extractor");
const base_1 = require("./base");
const clients_1 = require("./clients");
const solver_1 = require("./jsc/solver");
const pot_1 = require("./pot");
class YoutubeIE extends info_extractor_1.InfoExtractor {
    static IE_NAME = "youtube";
    static IE_DESC = "YouTube videos (watch, Shorts, live, music)";
    static _VALID_URL = /^https?:\/\/(?:www\.|m\.|music\.|gaming\.)?(?:youtube\.com\/(?:watch\?(?:[^#]*&)?v=|embed\/|v\/|shorts\/|live\/)|youtu\.be\/)|^[a-zA-Z0-9_-]{11}$/;
    static suitable(url) {
        return (0, base_1.validateURL)(url) || /^[a-zA-Z0-9_-]{11}$/.test(url.trim());
    }
    static getInfo() {
        return {
            name: this.IE_NAME,
    ...
    async extract(url) {
        const videoId = (0, base_1.getVideoID)(url);
        const potDirector = new pot_1.PoTokenDirector();
        if (this.params.poTokens) {
            potDirector.register(new pot_1.ManualPoTokenProvider(this.params.poTokens));
        }
        const watchUrl = `${base_1.YT_BASE}/watch?v=${videoId}&hl=${this.params.lang || "en"}&bpctr=${Math.ceil(Date.now() / 1000)}&has_verified=1`;
        const webpage = await this.request.text(watchUrl);
        const initialPr = (0, base_1.parseYtInitialPlayerResponse)(webpage);
        const ytcfg = (0, base_1.extractYtcfg)(webpage);
        let playerUrl = (0, base_1.extractPlayerJsUrl)(webpage);
        if (!playerUrl) {
            try {
                const embed = await this.request.text(`${base_1.YT_BASE}/embed/${videoId}?hl=${this.params.lang || "en"}`);
                playerUrl = (0, base_1.extractPlayerJsUrl)(embed);
            }
            catch {
                /* ignore */
            }
        }
        const visitorData = (0, base_1.extractVisitorData)(initialPr, ytcfg);
        let signatureTimestamp = null;
        if (playerUrl) {
            try {
                const playerJs = await this.request.text(playerUrl);
                signatureTimestamp = (0, base_1.extractSignatureTimestamp)(playerJs);
            }
            catch {
        ...
        for (const client of clients) {
            try {
                const playerPo = (await potDirector.getPoToken({
                    client,
                    context: "player",
                    videoId,
                    visitorData,
                ...
                const pr = await (0, base_1.callPlayerApi)(this.request, videoId, client, {
                    visitorData,
                    signatureTimestamp,
                    poToken: playerPo,
                });
                const wrongId = pr.videoDetails?.videoId;
                if (wrongId && wrongId !== videoId) {
                    continue;
                }
                const err = (0, base_1.playabilityError)(pr);
                if (err && !pr.streamingData) {
                    continue;
                }
        ...
        const playErr = (0, base_1.playabilityError)(primary);
        ...
        for (const { client, pr } of playerResponses) {
            ...
            if (sd.hlsManifestUrl) {
                formats.push({
                    ...
                    url: (0, pot_1.attachGvsPoToken)(sd.hlsManifestUrl, gvsPo),
            ...
            if (sd.dashManifestUrl) {
                formats.push({
                    ...
                    url: (0, pot_1.attachGvsPoToken)(sd.dashManifestUrl, gvsPo),
                    manifest_url: (0, pot_1.attachGvsPoToken)(sd.dashManifestUrl, gvsPo),
        ...
        if (playerUrl) {
            const nChallenges = [
                ...new Set(formatMeta.filter(m => m.needsN && m.rawN).map(m => m.rawN)),
            ];
            const sigChallenges = [
                ...new Set(formatMeta.filter(m => m.needsSig && m.rawS).map(m => m.rawS)),
            ];
            if (nChallenges.length || sigChallenges.length) {
                try {
                    const solver = new solver_1.NodeEjsChallengeSolver(this.request);
                    const reqs = [];
                    ...
                    if (!this.params.quiet) {
                        console.warn(`[youtube] JS challenge solving failed: ${err.message}`);
                    }
                }
            }
        }
        ...
        if (!formats.length) {
            throw new Error("No VLC-playable formats found. Try disabling “VLC-ready only”, or pass PO tokens if streams are gated.");
        }
        const details = (primary.videoDetails || {});
        const micro = (primary.microformat
        ...
        const info = {
            ...
            channel_url: details.channelId ? `${base_1.YT_BASE}/channel/${details.channelId}` : null,
            ...
            formats,
            webpage_url: `${base_1.YT_BASE}/watch?v=${videoId}`,
            original_url: url,
            extractor: "youtube",
    ...
    resolveClients() {
        ...
        return [...clients_1.VLC_CLIENTS];
    }
    parseStreamingFormat(raw, client, gvsPo) {
        let url = raw.url || "";
        let needsSig = false;
        let needsN = false;
        let rawN;
        ...
        url = (0, pot_1.attachGvsPoToken)(url, gvsPo);
        ...
        const format = {
            ...
            quality: typeof raw.quality === "string" ? undefined : raw.quality,
            format_note: raw.qualityLabel || raw.quality || client,
...
function normalizeClientName(name) {
    ...
    if (clients_1.INNERTUBE_CLIENTS[lower])
        return lower;
    const map = {
        web: "web",
...
function isVlcReady(f) {
    if (f.isHLS || f.protocol === "m3u8_native")
        return !!(f.url || f.manifest_url);
    if (!f.url)
        return false;
...
function extractThumbnails(details) {
