...
class LiveNewsNowIE extends info_extractor_1.InfoExtractor {
    ...
    static getInfo() {
        return {
            ...
            options: [],
            status: "ready",
            notes: "Discover all channels via `livenewsnow:categories` (default list) or a single `/category/american` / `/category/business` page, then extract `livenewsnow:foxnews` / channel page URLs. Streams include signed query params (`token`/`expires`/`sig` or `sec*`/`newz*`).",
            listSupported: true,
        };
    }
    static suitable(url) {
