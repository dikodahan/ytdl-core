...
class LiveNewsNowIE extends info_extractor_1.InfoExtractor {
    ...
    static getInfo() {
        return {
            ...
            options: [],
            status: "ready",
            notes: "Discover channels via `https://www.livenewsnow.com/category/american` or `/category/business`, then extract `livenewsnow:foxnews` / channel page URLs. Streams include signed query params (`token`/`expires`/`sig` or `sec*`/`newz*`).",
            listSupported: true,
        };
    }
    static suitable(url) {
