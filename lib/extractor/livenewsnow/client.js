...
exports.SIGNED_HLS_URL = exports.CATEGORY_PAGE_URL = exports.CHANNEL_PAGE_URL = exports.LNN_STREAM_HEADERS = exports.LNN_REQUEST_HEADERS = exports.LNN_CHANNEL_SECTIONS = exports.LNN_CATEGORIES = exports.LNN_HOME_URL = exports.LNN_ORIGIN = void 0;
...
function parseCategoryChannelsHtml(html, categoryId, pageUrl = lnnCategoryUrl(categoryId)) {
    ...
    const entryRe = /<h3[^>]*class="[^"]*entry-title[^"]*"[^>]*>\s*<a[^>]+href="([^"]+)"[^>]*(?:title="([^"]*)")?[^>]*>([\s\S]*?)<\/a>/gi;
    let m;
...
async function discoverLnnCategoryChannels(request, categoryId) {
    ...
    const html = await request.text(pageUrl, { headers: { ...exports.LNN_REQUEST_HEADERS } });
...
async function extractLnnStreamFromPage(request, pageUrl) {
    ...
    const html = await request.text(normalized, { headers: { ...exports.LNN_REQUEST_HEADERS } });
