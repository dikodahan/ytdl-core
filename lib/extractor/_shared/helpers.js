...
exports.parseHlsMasterPlaylist = parseHlsMasterPlaylist;
exports.expandHlsMasterFormats = expandHlsMasterFormats;
...
function parseHlsMasterPlaylist(masterUrl, body) {
    const variants = [];
 ...
 */
async function expandHlsMasterFormats(formats, fetchText) {
    const out = [];
    const seen = new Set();
    for (const format of formats) {
        ...
        try {
            ...
            for (const variant of variants) {
                if (seen.has(variant.playlistUrl))
                    continue;
                seen.add(variant.playlistUrl);
                out.push({
                    ...hlsFormat(variant.playlistUrl, `hls-${variant.name.replace(/\s+/g, "_").toLowerCase()}`),
