...
const VALID_URL = /^https?:\/\/(?:(?:(?:m|new|vksport)\.)?vk(?:(?:video)?\.ru|\.com)\/(?:.+?\?.*?z=)?(?:video|clip)|(?:www\.)?daxab\.com\/embed\/)(?<videoid>-?\d+_\d+)/i;
class VKIE extends info_extractor_1.InfoExtractor {
    static IE_NAME = "vk";
    static IE_DESC = "VK";
    ...
    async downloadPayload(path, data) {
        ...
        if (/\/challenge\.html/i.test(res.body) || res.statusCode === 429) {
            throw new Error("vk: received a JS challenge / rate-limit response — open the video in a browser or retry later");
        }
        const json = (0, helpers_1.tryParseJson)(res.body);
        if (!json?.payload) {
    ...
    async extract(url) {
        ...
        try {
            ...
            if (/login|challenge/i.test(msg))
                throw err;
            // Soft-fail path: try video_ext embed page
            infoPage = await this.request.text(`https://vk.com/video_ext.php?oid=${videoId.split("_")[0]}&id=${videoId.split("_")[1]}`);
            ...
            if (paramsMatch?.[1]) {
                player = (0, helpers_1.tryParseJson)(paramsMatch[1]) || {};
            }
        }
        const data = player.params?.[0];
        if (!data) {
            throw new Error(`vk: no player params for ${videoId} (deleted, private, or login required)`);
        }
        const formats = this.formatsFromPlayer(data);
        if (!formats.length) {
            throw new Error(`vk: no playable url*/hls* formats for ${videoId}`);
        }
        return (0, helpers_1.baseInfo)("vk", url, {
            id: videoId,
            ...
            uploader_id: data.author_id != null
