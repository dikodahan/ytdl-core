...
exports.SoundgasmIE = void 0;
const info_extractor_1 = require("../../core/info-extractor");
const helpers_1 = require("../_shared/helpers");
const VALID_URL = /^https?:\/\/(?:www\.)?soundgasm\.net\/u\/(?<user>[0-9a-zA-Z_-]+)\/(?<display_id>[0-9a-zA-Z_-]+)/i;
class SoundgasmIE extends info_extractor_1.InfoExtractor {
    ...
    async extract(url) {
        ...
        if (!audioUrl)
            throw new Error(`Could not find m4a URL on Soundgasm page ${displayId}`);
        const title = webpage.match(/<div[^>]+\bclass=["']jp-title[^>]+>([^<]+)/i)?.[1]?.trim() || displayId;
        const description = webpage.match(/<div[^>]+\bclass=["']jp-description[^>]+>([\s\S]+?)<\/div>/i)?.[1]
            ?.replace(/<[^>]+>/g, "")
            .trim() ||
        ...
        return (0, helpers_1.baseInfo)("soundgasm", url, {
            id: audioId,
...
exports.SoundgasmIE = SoundgasmIE;
//# sourceMappingURL=soundgasm.js.map